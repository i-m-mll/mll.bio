...
function getBlocks(markdown) {
  const tree = unified().use(remarkParse).use(remarkMdx, {skipExport: true}).parse(markdown)
  const blocks = []
  visit(tree, (node) => {
    ...
    const htmlSnippet = mdToHtml(mdSnippet)
...
function buildIndex() {
  const allDocs = []
  const store = {}
  if (!fs.existsSync(postsDirectory)) return { index: null, store: {} }
  ...
  files.forEach((fileName) => {
    ...
    blocks.forEach((block, idx) => {
      ...
      store[id] = { title, slug, snippetHtml: block.htmlSnippet, isCode: block.isCode, codeText: block.codeText, lang: block.lang }
    })
  })
